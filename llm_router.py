...
def review_top_candidates(candidates_data_list, news_data, fear_greed, macro_context="", market_bias_ctx=""):
    ...
    2. 📊 AGENTE 2 (Sniper de Suelo Fractal Confluente 1M/2M/5M/15M/30M/1H/2H): Exige entrada en ZONA BASE / SOPORTE (Canal 1M <= 40%, Canal 5M <= 50%, Canal 15M <= 55%, Canal 30M <= 58%, Canal 1H <= 60%, Canal 2H <= 65%). VETA categóricamente si el precio está en TECHO REAL de 1H/2H (Canal >= 70% con RSI >= 65) o si el activo es lento (ATR < 0.35%).
