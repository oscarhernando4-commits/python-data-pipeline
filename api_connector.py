...
def calculate_dynamic_proportional_trailing(highest_pnl_pct: float, atr_pct: float, holding_cycles: int = 0, current_pnl_pct: float = 0.0, custom_slack: float = None, symbol: str = None):
    ...
    try:
        ...
        # Fallback to standard 2-Phase Proportional Trailing (80% de retención de la cima máxima en Fase 2)
        retention_ratio = 0.80
        if highest_pnl_pct >= 0.50:
            sl_pct = round(highest_pnl_pct * retention_ratio, 2)
            phase = 2
            phase_label = f"🚀 FASE 2 TRAILING 80% (Cima +{highest_pnl_pct:.2f}% | Retención 80% -> Piso +{sl_pct:.2f}%)"
        else:
            sl_pct = -2.00
            phase = 1
